...
- Per-CLI template files: claude `settings.json`; codex `config.toml` + `models.json`; opencode `opencode.json`; pi `settings.json` + `models.json`; qwen `settings.json`; kimi `config.toml`; codebuddy `models.json`; crush `crushrc`; goose `config.yaml` + `custom-provider.json`. Model-level directories, when present, contain only the files needed for model-specific values or overrides.
